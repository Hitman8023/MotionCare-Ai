...
export default function Sidebar({ role, open, onNavigate }: SidebarProps) {
    ...
    return (
        <aside className={`sidebar${open ? ' sidebar-open' : ''}`}>
            ...
            {navItems.map((item) => (
                <NavLink key={item.to} to={item.to} end={item.to === '/'} onClick={onNavigate} className={({ isActive }) => `nav-item${isActive ? ' active' : ''}`}>
            ...
            <div className="sidebar-section-label">{role === 'doctor' ? 'Patient' : 'Support'}</div>
            {secondaryItems.map((item) => (
                <NavLink key={item.to} to={item.to} onClick={onNavigate} className={({ isActive }) => `nav-item${isActive ? ' active' : ''}`}>
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{item.icon}</svg>
                    {item.label}
                </NavLink>
            ))}
            ...
            <div className="sidebar-footer" style={{ marginTop: '20px' }}>
                ...
                <p>{role === 'doctor' ? 'Doctor mode enabled for patient management.' : 'Patient mode enabled for personal recovery tracking.'}</p>
            </div>
        </aside>
    );
}
